...
> Earlier revisions accepted one; it was removed because it caused real
> pain in Swagger UI (the literal `string` placeholder leaking through as
> a value) and added no signal the model couldn't recover from the image
> colour signature plus its own visual classification.
...
Detection is automatic and uses visual cues only: UI style, accent
colours, branding, and (for Deliveroo specifically) a server-side
teal-pixel colour signature that routes to the Deliveroo V2 prompt
before the image even reaches Gemini.
...
```javascript
async function extractFromBase64(base64String) {
