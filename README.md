...
> **Note when testing in Swagger UI:** the `hint` field will pre-fill with
> the literal placeholder text `string`. **Leave the hint field completely
> empty or type one of the supported values (e.g. `uber_eats`, `deliveroo`).
> Do not use the default `string` placeholder value** — it will be passed
> through as a real hint and treated as an unknown platform.
