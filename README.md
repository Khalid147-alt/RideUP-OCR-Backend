...
- ✅ **76 tests passing** — extraction, rate-limit handling, resilience, and end-to-end fixtures.
...
| `miles`      | `float \| null` | Distance in miles. **`null` is correct for Deliveroo V2** (see below).      |
...
| `confidence` | `string`        | `"high"` / `"medium"` / `"low"` — recomputed server-side from actual fields. |
| `notes`      | `string`        | Layout detected, conversions applied, retry markers, edge cases.            |
...
- **pytest + pytest-asyncio** — 76-test suite, all Gemini calls mocked
